  ...
  for (let item of items) {
    ...
    li.mousePressed(() => {
      if (item === '自我介紹') {
        ...
        showVideoWindow('https://cfchen58.synology.me/%E7%A8%8B%E5%BC%8F%E8%A8%AD%E8%A8%882024/B2/week4/20250310_104704.mp4'); // 點擊教學影片時顯示 iframe 視窗
      } else {
        alert(`${item} 被點擊了`);
      }
    });
...
function toggleDropdown(parentLi) {
  ...
  for (let subItem of subItems) {
    ...
    subLi.mousePressed(() => {
      if (subItem === '第一周') {
        showWeekWindow('https://wei10144.github.io/0303/'); // 點擊第一周時顯示 iframe 視窗
      } else if (subItem === '第二周') {
        showWeekWindow('https://wei10144.github.io/0310/'); // 點擊第二周時顯示 iframe 視窗
      } else if (subItem === '第三周') {
        showWeekWindow('https://wei10144.github.io/3/'); // 點擊第三周時顯示 iframe 視窗
      } else {
        alert(`${subItem} 被點擊了`);
      }
    });
    dropdown.child(subLi);
  }

  parentLi.parent().child(dropdown);
